...
namespace SkbKontur.Cassandra.DistributedTaskQueue.Tests.RemoteTaskQueue.RepositoriesTests
{
    public class TimeBasedBlobStorageTest : RepositoryFunctionalTestBase
    {
        ...
        {
            ResetCassandraState();
            var settings = new TimeBasedBlobStorageSettings(TestRtqSettings.QueueKeyspaceName, largeCfName, regularCfName);
        ...
        public void Write_Regular_WithLargeValue()
        {
            var id = RegularBlobId();
            var rng = new Random();
            var value = new byte[TimeBasedBlobStorageSettings.MaxRegularBlobSize + 1];
            rng.NextBytes(value);
