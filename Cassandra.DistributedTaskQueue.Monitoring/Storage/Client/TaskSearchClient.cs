...
public class TaskSearchClient
{
    ...
    public TaskSearchResponse Search(TaskSearchRequest taskSearchRequest, int from, int size)
    {
        ...
        var body = PostData.String(JsonSerializer.Serialize(request));
        var stringResponse = elasticClient.Search<StringResponse>(indexForTimeRange, body, ignoreUnavailableIndices).EnsureSuccess();
        var searchResponse = JsonSerializer.Deserialize<SearchResponse>(stringResponse.Body);
    ...
    private readonly SearchRequestParameters ignoreUnavailableIndices = new()
        {
